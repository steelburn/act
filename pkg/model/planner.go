	...
	for _, wf := range workflows {
		...
		if ext == ".yml" || ext == ".yaml" {
			...
			if err != nil {
				_ = f.Close()
				if err == io.EOF {
					return nil, fmt.Errorf("unable to read workflow %q, file is empty: %w", wf.workflowFileInfo.Name(), err)
				}
				return nil, err
			}
			_, err = f.Seek(0, 0)
			if err != nil {
				_ = f.Close()
				return nil, fmt.Errorf("error occurring when resetting io pointer in %q: %w", wf.workflowFileInfo.Name(), err)
			}
